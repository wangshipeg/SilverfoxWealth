        ...
        if (self.message.messageType == UDMessageContentTypeText) {
            ...
            switch (self.message.messageFrom) {
                case UDMessageTypeSending:{
                    //文本气泡frame
                    self.bubbleFrame = CGRectMake(self.avatarFrame.origin.x - kUDArrowMarginWidth -kUDBubbleToTextHorizontalSpacing*2 - kUDAvatarToBubbleSpacing-textSize.width, self.avatarFrame.origin.y, textSize.width+(kUDBubbleToTextHorizontalSpacing*3), textSize.height+kUDBubbleToTextVerticalSpacing);
                    //文本frame
                    self.textFrame = CGRectMake(kUDBubbleToTextHorizontalSpacing,kUDTextMendSpacing, textSize.width, textSize.height);
                    //加载中frame
                ...
                case UDMessageTypeReceiving:{
                    //接收文字气泡frame
                    self.bubbleFrame = CGRectMake(self.avatarFrame.origin.x+kUDAvatarDiameter+kUDAvatarToBubbleSpacing, self.dateFrame.origin.y+self.dateFrame.size.height+kUDAvatarToVerticalEdgeSpacing, textSize.width+(kUDBubbleToTextHorizontalSpacing*3), textSize.height+kUDBubbleToTextVerticalSpacing);
                    //接收文字frame
                    self.textFrame = CGRectMake(kUDBubbleToTextHorizontalSpacing+kUDArrowMarginWidth, kUDTextMendSpacing, textSize.width, textSize.height);
                    break;
                }
                default:
                    break;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //接收文字气泡frame
                self.bubbleFrame = CGRectMake(self.avatarFrame.origin.x+kUDAvatarDiameter+kUDAvatarToBubbleSpacing, self.dateFrame.origin.y+self.dateFrame.size.height+kUDAvatarToVerticalEdgeSpacing, richTextSize.width+(kUDBubbleToTextHorizontalSpacing*3), richTextSize.height+kUDBubbleToTextVerticalSpacing - 5);
                //接收文字frame
                self.textFrame = CGRectMake(kUDBubbleToTextHorizontalSpacing+kUDArrowMarginWidth, kUDTextMendSpacing, richTextSize.width, richTextSize.height);
                //cell高度
                self.cellHeight = self.bubbleFrame.size.height+self.bubbleFrame.origin.y +kUDCellBottomMargin;
            });
        }
...
- (CGSize)setAttributedCellText:(NSString *)text messageFrom:(UDMessageFromType)messageFrom {
    ...
    @try {
        ...
        NSMutableDictionary *contentAttributes = [[NSMutableDictionary alloc] initWithDictionary:@{
