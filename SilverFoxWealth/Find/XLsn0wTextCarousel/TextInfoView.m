...
//    self.bottomLabel.font = [UIFont systemFontOfSize:13];
    ...
    [self.topButton setFrame:(CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width - 90, 60))];
    [self.topButton addTarget:self action:@selector(topButtonEvent:) forControlEvents:(UIControlEventTouchUpInside)];
    self.topButton.titleLabel.lineBreakMode =  NSLineBreakByTruncatingTail;
    self.topButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentFill;
    [self.topButton setTitleColor:[UIColor characterBlackColor] forState:(UIControlStateNormal)];
