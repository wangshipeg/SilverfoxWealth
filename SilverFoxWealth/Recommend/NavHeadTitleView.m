...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self=[super initWithFrame:frame]) {
        ...
        if (IS_iPhoneX) {
            label.frame = CGRectMake(0, 87.5, [UIScreen mainScreen].bounds.size.width, 0.5);
        }else{
            label.frame = CGRectMake(0, 63.5, [UIScreen mainScreen].bounds.size.width, 0.5);
        }
        
        label.backgroundColor = [UIColor typefaceGrayColor];
        ...
        if (IS_iPhoneX) {
            self.back.frame=CGRectMake(5, 44, 44, 44);
        }else{
        ...
        if (IS_iPhoneX) {
            self.label=[[UILabel alloc]initWithFrame:CGRectMake(44, 44, frame.size.width-44-44, 44)];
        } else {
            self.label=[[UILabel alloc]initWithFrame:CGRectMake(44, 20, frame.size.width-44-44, 44)];
        }
        self.label.textAlignment=NSTextAlignmentCenter;
        self.label.font = [UIFont systemFontOfSize:16];
        _label.textColor = [UIColor characterBlackColor];
