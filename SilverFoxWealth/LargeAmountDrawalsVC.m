...
@interface LargeAmountDrawalsVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) UITextField *usableAssetTF;
@property (nonatomic, strong) UITextField *bankInfoTF;
@property (nonatomic, strong) UITextField *openBankNOTF;
@property (nonatomic, strong) UIButton *queryBankNOBT;
...
@property (nonatomic, strong) NSString *lineNOStr;
@property (nonatomic, strong) UILabel *infoLB;
@property (nonatomic, assign)  double amount;
@end

@implementation LargeAmountDrawalsVC
...
- (void)addMessageNoteWith
{
    if (_freeTimesStr) {
        ...
        _drawalsTime.text = @"0.00元";
    }
    [self checkQueryBankNOBT:_queryBankNOBT];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 2) {
        AddBankCardCell *cell = [[AddBankCardCell alloc] initWithTitle:@"开户支行" inputTF:_openBankNOTF leftViewWidth:90];
        _queryBankNOBT = [UIButton buttonWithType:UIButtonTypeCustom];
        _queryBankNOBT.frame = CGRectMake(-10, 0, 80, 30);
        [_queryBankNOBT setTitle:@"查询联行号" forState:UIControlStateNormal];
        [_queryBankNOBT setTitleColor:[UIColor iconBlueColor] forState:UIControlStateNormal];
        [_queryBankNOBT addTarget:self action:@selector(checkQueryBankNOBT:) forControlEvents:UIControlEventTouchUpInside];
        _queryBankNOBT.titleLabel.font=[UIFont systemFontOfSize:14];
        _queryBankNOBT.titleLabel.textAlignment=NSTextAlignmentCenter;
        [_openBankNOTF setRightView:_queryBankNOBT];
        _openBankNOTF.rightViewMode=UITextFieldViewModeAlways;
        cell.selectionStyle =UITableViewCellSelectionStyleNone;
        return cell;
    }
    if (indexPath.row == 3) {
...
- (void)UIDecorate {
    ...
    _usableAssetTF.font = [UIFont systemFontOfSize:14];
    _usableAssetTF.textAlignment = NSTextAlignmentRight;
    if (_balanceStr) {
        _usableAssetTF.text = [NSString stringWithFormat:@"%@元",_balanceStr];
    } else {
    ...
    _openBankNOTF = [[UITextField alloc] init];
    _openBankNOTF.font = [UIFont systemFontOfSize:14];
    _openBankNOTF.textColor = [UIColor characterBlackColor];
    _openBankNOTF.keyboardType = UIKeyboardTypeNumberPad;
    UIView *openBankNOTFDis=[AddCancelButton addCompleteBTOnVC:self withSelector:@selector(drawalsOpenBankNOTFDisrespond) title:@"完成"];
    [_openBankNOTF setInputAccessoryView:openBankNOTFDis];
...
- (void)checkSureBT:(RoundCornerClickBT *)sender
{
    ...
    [[DataRequest sharedClient]requsetPaymentsWithdrawWithCustomerId:user.customerId accountId:user.accountId provinceBankNO:_lineNOStr cardNO:_cardNOStr principal:_drawalsAmountTF.text channel:@"000001" detailChannel:@"2" callback:^(id obj) {
        if ([obj isKindOfClass:[NSDictionary class]]) {
            if ([obj[@"code"] intValue] == 2000) {
                CommitDrawalsInfoWebView *commitView = [[CommitDrawalsInfoWebView alloc] init];
                commitView.provinceBankNO = _openBankNOTF.text;
...
- (void)checkQueryBankNOBT:(UIButton *)sender
{
    [MobClick event:@"click_get_voucher"];
    [[DataRequest sharedClient] requestBankLinenoWithCardNO:_cardNOStr callback:^(id obj) {
        DLog(@"查询联行号======%@",obj);
        if ([obj isKindOfClass:[NSDictionary class]]) {
            _lineNOStr = obj[@"voucher"];
             _openBankNOTF.text = _lineNOStr;
        }
...
- (void)detectionUserInpute {    
    if (self.drawalsAmountTF.text.length > 0 && _openBankNOTF.text.length > 1 && _bankInfoStr) {
...
- (void)detectionDrawalsMoneyTFText:(NSNotification *)note
{
    ...
    if ([_freeTimesStr intValue] > 0) {
        _amount = 0.00;
        _factorageTF.text = @"0.00元";
        _realityArrivalTF.text = [NSString stringWithFormat:@"%@元",_drawalsAmountTF.text];
    }else{
...
- (void)drawalsOpenBankNOTFDisrespond
{
    [self.openBankNOTF resignFirstResponder];
}
