...
@interface AllExchangeRecordVC ()<UITableViewDelegate,UITableViewDataSource>
{
    int page;
}
...
@implementation AllExchangeRecordVC
...
- (void)achieveExchangeRecordDataWith:(NSString *)customerId {
    [[DataRequest sharedClient] requestExchageDetailWithCustomerId:customerId type:-1 page:page history:0 mobile:1 startDate:nil endDate:nil callback:^(id obj) {
        DLog(@"交易明细返回结果======%@",obj);
        ...
        [self sendMessageToIncomeRecordDataRequest];
    }];
}
- (void)sendMessageToIncomeRecordDataRequest{
    NSNotificationCenter *messageCenter=[NSNotificationCenter  defaultCenter];
    [messageCenter postNotificationName:@"AllExchangeLoadOver" object:Nil userInfo:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *silverIdentifier=@"exchangeRecord";
...
- (void)notHaveSilverClearList {
    ...
    [VCAppearManager arrengmentNotDataViewWithSuperView:_backContentView title:@"土豪，暂无交易记录哦!"];
